...
${player.get('itemsText')}`;
    ...
    scope.bfigga552 = () => {
      ...
      var text = `The structure is hewn of smooth stone, although in some places it is cracked and weathered.  The door is open. A parapet encircles the roof, and is sloped outwards to prevent would-be climbers from gaining untoward access.`;
    ...
    scope.iin7r5kkb = () => {
      ...
      var text = `A massive structure fills this cavern, with a stairway that leads up to a massive, closed door.
...
${player.get('itemsText')}`;
  ...
  files[`Caves_SecretCave1.json`] = function (id) {
    ...
    scope.g854qy2nf = () => {
      ...
      var text = `It is too dark to make out anything in this cave, but it feels hot and muggy. Nearby the mouth of this cave appears to be a doused campfire.
...
${player.get('itemsText')}`;
    ...
    scope.bncwgxo0a = () => {
      ...
      var text = `With the help of the tinderbox you manage to light the remaining wood in the campfire.  It lights up the room so you can see much better.`;
    ...
    scope.azd7ggfkb = () => {
      ...
      var text = `A deliberate circle of rocks around a stack of charred wood marks a campfire at the mouth of the cave.  It looks like it was doused before all the wood got a chance to burn; some of it has not turned to charcoal yet and could maybe still be re-lit.
...
There are no other clues to be found around the campfire. It is not clear who made it or why it is here, nor how long ago that the fire had been lit.  The wood is not warm to the touch.`;
    ...
    scope.bfq3nepnr = () => {
      ...
      var text = `The cliff face is tall and sheer, with little to no handholds that you can use to climb it.
...
When you look up, you see that a rope has been tied to a rock at the top of the cliff.  However it has been cut so it barely dangles an arm-length over the edge.  There is no way for you to reach it, and you don't see any signs of the other end of the cut-off piece of rope.`;
    ...
    scope.hzdao4hgy = () => {
      ...
      var text = `You stand at the base of a sheer cliff face.  It is hot and muggy, and the ceiling extends higher than you can see.  The lit campfire flickers light that casts long shadows on the walls.
...
There are exits to the NORTH, EAST, and WEST.`;
    ...
    scope.efue8dmfz = () => {
      ...
      var text = `You pull out the RUSTY AXE and wedge it into a gap between the cave wall and a large boulder.  Throwing your whole weight on it, you manage to shift the rock about an inch.`;
...
It is a miracle you did not get hurt.`;
    ...
    scope.ot575owxg = () => {
      ...
      var text = `You find a suitable boulder and grab on, but even after much heaving and hoeing it won't budge at all.  You do not have enough leverage.`;
...
${player.get('itemsText')}`;
    ...
    scope.rxngrz6ro = () => {
      ...
      var text = `Near the edge of the window is the handle of a small RUSTY AXE.  You might be able to reach it if you squeeze your arm through the bars.`;
    ...
    scope.f4kwmr8wz = () => {
      ...
      var text = `You budge up next to the window and stick your arm through up to your shoulder.  With some targeted flailing, you manage to budge the RUSTY AXE towards the window.`;
...
${player.get('itemsText')}`;
  ...
  files[`Caves_BelowDungeon.json`] = function (id) {
    ...
    scope.ecg8lgliw = () => {
      ...
      var text = `You see a light coming from a dilapidated hole high up in the ceiling.  Did you fall from up there?  You have a faint memory of having been up there, but it is too vague to be certain.  For some reason your memory is very fuzzy.`;
...
You have ${
  ...
  files[`TestRoom.json`] = function (id) {
    ...
    // switch
    scope.s4am0gxgy = () => {
      player.set(CURRENT_NODE_VAR, 's4am0gxgy');
      if (false) scope.y0tqnbffn();
      else if (true) scope.orpocs4op();
    };
    // action
    scope.y0tqnbffn = () => {
      player.set(CURRENT_NODE_VAR, 'y0tqnbffn');
      player.set('itemsText', engine.getRoomItemsText());
      scope.ciw1mnze8();
    };
    ...
    // text
    scope.ciw1mnze8 = () => {
      player.set(CURRENT_NODE_VAR, 'ciw1mnze8');
      var text = `<Succinct Description>.

${player.get('exitText')}

${player.get('itemsText')}`;
      core.say(text, scope.enlsrgds9, 'ciw1mnze8', 'enlsrgds9');
    };
    scope['ciw1mnze8'].isText = true;

    // choice
    scope.enlsrgds9 = function () {
      player.set(CURRENT_NODE_VAR, 'enlsrgds9');
      let text = ``;
      core.choose(text, 'enlsrgds9', [
        {
          t: `Pick up items.`,
          id: 'sik193uau',
          cb: scope.sik193uau,
    ...
    scope['enlsrgds9'].isChoice = true;

    // action
    scope.sik193uau = () => {
      player.set(CURRENT_NODE_VAR, 'sik193uau');
      player.set('PICKUP_NEXT_FILE_ID', player.get('curIN2f'));
      player.set('PICKUP_NEXT_NODE_ID', 'si1q1rpai');
      scope.trneea9zr();
    };
    ...
    // action
    scope.orpocs4op = () => {
      player.set(CURRENT_NODE_VAR, 'orpocs4op');
      engine.setBackground('Caves_Cave1');
      engine.setHeading('n');
    ...
    // text
    scope.gg7rmef6f = () => {
      player.set(CURRENT_NODE_VAR, 'gg7rmef6f');
      var text = `<Title>`;
      core.say(text, scope.ccw1n26q7, 'gg7rmef6f', 'ccw1n26q7');
    };
    scope['gg7rmef6f'].isText = true;

    // choice
    scope.ccw1n26q7 = function () {
      player.set(CURRENT_NODE_VAR, 'ccw1n26q7');
      let text = ``;
      core.choose(text, 'ccw1n26q7', [
        {
          t: `Examine surroundings.`,
          id: 'uf00lcppy',
          cb: scope.uf00lcppy,
    ...
    scope['ccw1n26q7'].isChoice = true;

    // action
    scope.uf00lcppy = () => {
      player.set(CURRENT_NODE_VAR, 'uf00lcppy');
      player.set('itemsText', engine.getRoomItemsText());
    ...
    // text
    scope.rbyk9efr4 = () => {
      player.set(CURRENT_NODE_VAR, 'rbyk9efr4');
      var text = `<Primary Description>.

${player.get('exitText')}

${player.get('itemsText')}asdf`;
      core.say(text, scope.enlsrgds9, 'rbyk9efr4', 'enlsrgds9');
    };
    scope['rbyk9efr4'].isText = true;

    // action
    scope.btm4wtk59 = () => {
      player.set(CURRENT_NODE_VAR, 'btm4wtk59');
      player.set('INVENTORY_NEXT_NODE_ID', 'p1rd41fiq');
      player.set('INVENTORY_NEXT_FILE_ID', player.get(CURRENT_FILE_VAR));
      scope.gmqa92rsk();
    };
  ...
  // sub_root
  scope.ncrotkutk = () => {
    player.set(CURRENT_NODE_VAR, 'ncrotkutk');
    scope.k64xom9sz();
  };

  // text
  scope.k64xom9sz = () => {
    player.set(CURRENT_NODE_VAR, 'k64xom9sz');
    var text = `This node currently has no actual content.`;
    core.say(text, scope.kcfoa4rat, 'k64xom9sz', 'kcfoa4rat');
  };
  scope['k64xom9sz'].isText = true;

  // next_file
  scope.kcfoa4rat = function () {
    var key = `Caves_BelowCellar.json`;
    var func = files[key];
    if (!func) {
      func = files[player.get(key)];
